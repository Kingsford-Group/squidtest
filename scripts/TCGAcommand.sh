...
for ((i=0; i<${#argv}; i+2)); do
	if [[ ${argv[i]} == "-p" ]]; then
		...
		sample_1_fastq=${argv[((i+1))]}
	elif [[ ${argv[i]} == "-r2" ]]; then
		sample_2_fastq=${argv[((i+1))]}
	elif [[ ${argv[i]} == "-i" ]]; then
		StarIndex=${argv[((i+1))]}
	elif [[ ${argv[i]} == "-g" ]]; then
		GTFfile=${argv[((i+1))]}
	elif [[ ${argv[i]} == "-o" ]]; then
		sample_Output=${argv[((i+1))]}
	else; then
...
python3 src/FilterChimOut.py ${sample_Output}/speedseqChim/Aligned.bam ${sample_Output}/Chimeric.out.bam ${sample_Output}/Chimeric.out.filtered.bam
